...
impl Plugin for MainPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(AnimationTimer(Timer::from_seconds(1_f32, TimerMode::Repeating)));
        app.insert_resource(SelectedPiece(None));
        app.add_systems(Startup, create);
        app.add_systems(Update, (keyboard_update));
    }
}
...
fn create(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>, mut materials: ResMut<Assets<StandardMaterial>>) {
    commands.spawn((
        Transform::default(),
        Rotator,
        children![(
    ...
    for col in 0..board_size{
        for row in 0..board_size{
            ...
            if row < 2 || row > 5 {
                    ...
                    if row > 5 {
                        piece_color  = Color::srgb(0.8, 0.8, 0.8);
                    } else {
                        piece_color  = Color::srgb(0.2, 0.2, 0.2);
                    }

                    commands.spawn((
                        ChessPieces,
                        Mesh3d(meshes.add(Cuboid::new(0.6, 1.0, 0.6))),
                        MeshMaterial3d(materials.add(piece_color)),
                        Transform::from_xyz(board_part_x, 1.0, board_part_y)
...
fn handle_click(
    ...
    if let Some(selected_piece_enity) = selected_piece.0 {
        ...
        if let Ok([hover_transform, mut selected_transform]) = transform_query.get_many_mut([hovered_entity, selected_piece_enity]){
            ...
            selected_transform.translation.y = 1.0;
